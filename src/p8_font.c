 ...
 *
 * 3×5 glyphs in a 4×6 cell. ASCII 32..127 fully populated; lowercase
 * has its own glyphs (descenders for g/p/q/y/j visible in row 5).
 *
 * Glyph bitmap data was transcribed from Pemsa
 * (https://github.com/egordorichev/pemsa, MIT license) by parsing
 * its ASCII-art font table and re-encoding into the same 16-bit
 * row-major format ThumbyP8 uses internally:
 *
...
static uint16_t lookup_glyph(unsigned char ch) {
    if (ch >= 128) return glyph_unknown;
    uint16_t g = font[ch];
    return g;  /* 0 (= empty) is fine for space and unknowns */
}
